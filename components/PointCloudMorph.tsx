...
export default function PointCloudMorph({
  ...
  const generatePointCloud = (
    ...
    gltf.scene.traverse((child: any) => {
      if (child.isMesh) {
        ...
        const colorsArray = colorAttribute ? (colorAttribute.array as Float32Array) : null
        const indicesArray = indexAttribute ? (indexAttribute.array as Uint16Array | Uint32Array) : null
        ...
        const triangleCount = indexAttribute ? indexAttribute.count / 3 : positionsArray.length / 9
        ...
        for (let tri = 0; tri < triangleCount; tri++) {
          ...
          const v1 = new THREE.Vector3(positionsArray[i1], positionsArray[i1 + 1], positionsArray[i1 + 2])
          const v2 = new THREE.Vector3(positionsArray[i2], positionsArray[i2 + 1], positionsArray[i2 + 2])
          const v3 = new THREE.Vector3(positionsArray[i3], positionsArray[i3 + 1], positionsArray[i3 + 2])
          ...
          for (let j = 0; j < numPoints; j++) {
            ...
            if (colorsArray && colorsArray.length > 0) {
              const color1 = new THREE.Color(colorsArray[i1], colorsArray[i1 + 1], colorsArray[i1 + 2])
              const color2 = new THREE.Color(colorsArray[i2], colorsArray[i2 + 1], colorsArray[i2 + 2])
              const color3 = new THREE.Color(colorsArray[i3], colorsArray[i3 + 1], colorsArray[i3 + 2])
  ...
  // 2つの点群間で補間する関数
  const interpolatePointClouds = (geom1: THREE.BufferGeometry, geom2: THREE.BufferGeometry, t: number): THREE.BufferGeometry => {
    ...
    for (let i = 0; i < maxLength; i++) {
      ...
      const p1 = new THREE.Vector3(pos1.getX(idx1), pos1.getY(idx1), pos1.getZ(idx1))
      const p2 = new THREE.Vector3(pos2.getX(idx2), pos2.getY(idx2), pos2.getZ(idx2))

      const p = new THREE.Vector3().lerpVectors(p1, p2, t)
      positions.push(p.x, p.y, p.z)

      const c1 = new THREE.Color(color1.getX(idx1), color1.getY(idx1), color1.getZ(idx1))
      const c2 = new THREE.Color(color2.getX(idx2), color2.getY(idx2), color2.getZ(idx2))
  ...
  useEffect(() => {
    ...
    Promise.all([loader.loadAsync(model1Path), loader.loadAsync(model2Path)])
      .then(([gltf1, gltf2]) => {
        // console.log('モデル読み込み成功', { gltf1, gltf2 })
        try {
          const {geometry: geom1, colors: colors1} = generatePointCloud(gltf1, pointDensity, model1Position)
          const {geometry: geom2, colors: colors2} = generatePointCloud(gltf2, pointDensity, model2Position)
  ...
  useEffect(() => {
    if (pointsRef.current && points1 && points2 && morphProgress !== lastProgressRef.current) {
      const interpolated = interpolatePointClouds(points1, points2, morphProgress)
  ...
  if (pointCount === 0) {
    ...
    for (let i = 0; i < 100; i++) {
      testPositions.push((Math.random() - 0.5) * 6 - 3, (Math.random() - 0.5) * 2, (Math.random() - 0.5) * 2)
    ...
    return (
      <points geometry={testGeometry}>
        <pointsMaterial vertexColors size={pointSize * 2} sizeAttenuation={true} />
      </points>
    )
  }
  ...
  return (
    <points ref={pointsRef} geometry={geometryRef.current}>
      <pointsMaterial vertexColors size={pointSize} transparent={false} opacity={1} sizeAttenuation={true} depthWrite={true} />
    </points>
  )
}
